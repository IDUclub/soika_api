...
async def get_social_risk(
    ...
    response = await risk_calculator.calculate_social_risk(dto.territory_id, dto.project_id)
    return response
...
async def get_social_risk_coverage(
    ...
    logger.info(f"Started request processing with territory_id={dto.territory_id}, project_id={dto.project_id}")
    TEXTS.try_init(bucket_name, text_name)
    response = await risk_calculator.calculate_coverage(dto.territory_id, dto.project_id)
...
async def get_named_objects(
    ...
    response = await risk_calculator.collect_named_objects(dto.territory_id, dto.project_id)
