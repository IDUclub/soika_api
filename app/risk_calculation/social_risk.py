...
class RiskCalculationService:
    @staticmethod
    async def get_social_risk(territory_id: int, project_id: int) -> dict:
        logger.info(f"Service: Calculating social risk for territory {territory_id} and project {project_id}")
        return await risk_calculation.calculate_social_risk(territory_id, project_id)

    @staticmethod
    async def get_risk_coverage(territory_id: int, project_id: int) -> dict:
        logger.info(f"Service: Calculating risk coverage for territory {territory_id} and project {project_id}")
        return await coverage_calculation.calculate_coverage(territory_id, project_id)

    @staticmethod
    async def collect_texts(territory_id: int, project_id: int, time_period: str) -> dict:
        logger.info(f"Service: Collecting texts for territory {territory_id}, project {project_id} for period {time_period}")
        return await text_processing.collect_texts(territory_id, project_id, time_period)

    @staticmethod
    async def generate_risk_values(territory_id: int, project_id: int) -> dict:
        logger.info(f"Service: Generating risk values for territory {territory_id} and project {project_id}")
        return await risk_values_collection.calculate_values_to_risk_data(territory_id, project_id)
    ...
    @staticmethod
    async def get_named_objects(territory_id: int, project_id: int) -> dict:
        logger.info(f"Service: Collecting named objects for territory {territory_id} and project {project_id}")
        return await named_objects_collection.collect_named_objects(territory_id, project_id)
