...
class RiskCalculation:
    ...
    async def get_texts(territory_gdf: gpd.GeoDataFrame):
        ...
        return {"texts": local_texts.to_crs(epsg=4326)}

    async def calculate_score(self, df):
        df = df.copy()
        df["emotion_weight"] = df["emotion"].map(self.emotion_weights)
        df["minmaxed_views"] = (df["views.count"] - df["views.count"].min()) / (
            df["views.count"].max() - df["views.count"].min()
        )
        df["minmaxed_likes"] = (df["likes.count"] - df["likes.count"].min()) / (
            df["likes.count"].max() - df["likes.count"].min()
        )
        df["minmaxed_reposts"] = (df["reposts.count"] - df["reposts.count"].min()) / (
            df["reposts.count"].max() - df["reposts.count"].min()
        )
        df["engagement_score"] = df.fillna(0).apply(
            lambda row: row["minmaxed_views"]
            + row["minmaxed_likes"]
            + row["minmaxed_reposts"]
            + 1,
            axis=1,
        )
        ...
        df["activity_level"] = df["engagement_score"].apply(
            lambda x: "активно"
            if x >= high_threshold
            else ("умеренно" if x >= low_threshold else "мало")
        )
    ...
    def generate_description(row):
        ...
        if indicator_count == 1:
            indicators_text = f"Среди показателей оценки уровня общественного резонанса выделяется один ключевой - {indicators_list[0]} сервиса данного типа."
        ...
        activity_text = f"Сервис {activity_level} обсуждается пользователями."
        emotion_mapping = {
            "negative": "негативную",
        ...
        emotion_text = f"Эмоциональную окраску дискуссии можно охарактеризовать как преимущественно {emotion_descr}."
        base_priorities = {"indicators": 2, "activity": 1, "emotion": 1}
        ...
        sorted_other_blocks = sorted(
            other_blocks.items(), key=lambda x: x[1][0], reverse=True
        )
    ...
    async def score_table(df):
        score_df = (
            df.groupby(["services", "indicators"])["score"].mean().unstack(fill_value=0)
        )
        score_df_numeric = score_df.copy()
        score_df["top_indicators"] = score_df_numeric.apply(
            risk_calculator.get_top_indicators, axis=1
        )
        score_df["risk_rating"] = (
            score_df_numeric.sum(axis=1).clip(upper=5).round(0).astype(int)
        )
        score_df["risk_level"] = score_df["risk_rating"].map(
            lambda x: "высокой" if x >= 4 else ("средней" if 2 <= x < 4 else "низкой")
        )

        emotion_table = df.groupby("services")["emotion"].agg(
            lambda x: x.mode().iloc[0]
        )
        activity_table = df.groupby("services")["activity_level"].agg(
            lambda x: x.mode().iloc[0]
        )

        final_table = score_df.join(emotion_table)
        final_table = final_table.join(activity_table)
        final_table["description"] = final_table.apply(
            risk_calculator.generate_description, axis=1
        )
    ...
    async def calculate_social_risk(self, territory_id, project_id):
        ...
        project_area = await urban_db_api.get_context_territories(
        ...
        texts_df = (
            texts_df.groupby(["text", "services"])
            .agg({"indicators": lambda x: ", ".join(set(x))})
            .reset_index()
        )
        result_df = (
            texts_df.groupby("services")
            .agg({"text": lambda x: list(x), "indicators": lambda x: list(x)})
            .reset_index()
        )
    ...
    async def fetch_municipalities(df):
        ...
        if len(territories_to_fetch) == 0:
            return df
        tasks = [
            risk_calculator.get_level_4_territory(territory_id)
            for territory_id in territories_to_fetch["territory_id"]
        ]
        results = await asyncio.gather(*tasks)
        final_df = gpd.GeoDataFrame(
            pd.concat(results, ignore_index=True), geometry="geometry", crs=4326
        )
    ...
    def generate_category_table(self):
        ...
        mapping_df["category"] = mapping_df["category"].map(
            risk_calculator.generate_value_names
        )
    ...
    async def calculate_values_to_risk_data(self, territory_id, project_id):
        ...
        logger.info(
            f"Calculating risk-to-value table for project {project_id} and its context"
        )
        scored_texts = await risk_calculator.calculate_score(texts["texts"])
        ...
        values_to_risk_table["Общественный резонанс"] = values_to_risk_table[
            "Общественный резонанс"
        ].fillna(0)
        values_to_risk_table.reset_index(inplace=True)
        values_to_risk_table.rename(columns={"index": "category"}, inplace=True)
        values_to_risk_table["category"] = values_to_risk_table["category"].map(
            risk_calculator.generate_value_names
        )
        ...
        values_to_risk_table = values_to_risk_table.merge(
            category_table, on="category", how="left"
        )
        values_to_risk_table["services"] = values_to_risk_table["services"].fillna(
            "Нет данных по сервисам"
        )
        response = {
            "values_to_risk_table": values_to_risk_table.to_dict(orient="records")
        }
        logger.info(f"Table response generated")
        return response
    ...
    @staticmethod
    async def get_areas(
        urban_areas: gpd.GeoDataFrame, texts: gpd.GeoDataFrame
    ) -> gpd.GeoDataFrame:
        ...
        urban_areas = urban_areas[
            ["name", "territory_id", "admin_center", "is_city", "geometry", "count"]
        ]
        ...
        urban_areas = urban_areas.sort_values(
    ...
    async def get_links(
        ...
        region_territories: gpd.GeoDataFrame,
    ) -> gpd.GeoDataFrame:
        ...
        areas["geometry"] = await asyncio.gather(
            *[
                risk_calculator.get_area_centroid(area, region_territories)
        ...
        grouped = (
            areas.groupby("geometry").agg({"name": lambda x: list(x)}).reset_index()
        )
        grouped.rename(columns={"name": "urban_area"}, inplace=True)
        combined_lines_gdf = gpd.GeoDataFrame(
            grouped, geometry="geometry", crs=areas.crs
        )
    ...
    async def calculate_coverage(self, territory_id, project_id):
        ...
        urban_areas = await risk_calculator.get_areas(
            region_territories, texts["texts"]
        )
        logger.info(f"Generating links from project {project_id} to coverage areas")
        links = await risk_calculator.get_links(
            project_id, urban_areas, region_territories
        )
    ...
    async def collect_named_objects(self, territory_id, project_id):
        ...
        response = {"named_objects": json.loads(local_objects.to_json())}
        return response
