...
from app.risk_calculation.preprocessing_controller import (
    territories_router,
    groups_router,
    messages_router,
...
app.include_router(
    territories_router, prefix=config.get("FASTAPI_PREFIX"), tags=["Territories"]
)
app.include_router(groups_router, prefix=config.get("FASTAPI_PREFIX"), tags=["Groups"])
app.include_router(
    messages_router, prefix=config.get("FASTAPI_PREFIX"), tags=["Messages"]
)
app.include_router(
    named_objects_router, prefix=config.get("FASTAPI_PREFIX"), tags=["Named objects"]
)
app.include_router(
    indicators_router, prefix=config.get("FASTAPI_PREFIX"), tags=["Indicators"]
)
app.include_router(
    services_router, prefix=config.get("FASTAPI_PREFIX"), tags=["Services"]
)
