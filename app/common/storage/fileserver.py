...
class DataGetter:
    ...
    def get_pickle(object_name: str, received_bytes: io.BytesIO | None) -> DataFrame:
        if received_bytes is None:
            received_df = pd.read_pickle(
                caching_service.cache_path
                / caching_service.get_cached_file_name(object_name)
            )
            logger.info(
                f"Loaded pickle from cache - {caching_service.get_cached_file_name(object_name)}"
            )
            return received_df

        caching_service.save(
            CacheableBytes(received_bytes), object_name, datetime.now()
        )
    ...
    def get_json(object_name: str, received_bytes: io.BytesIO | None) -> dict:
        if received_bytes is None:
            with io.open(
                ...
                logger.info(
                    f"Loaded json from cache - {caching_service.get_cached_file_name(object_name)}"
                )
    ...
    def get_bytes(bucket_name: str, object_name: str) -> io.BytesIO | None:
        ...
        try:
            meta = client.stat_object(bucket_name=bucket_name, object_name=object_name)
        ...
        if (
            ...
            or (
                server_file_date
                - datetime.strptime(existing_file_date, "%Y-%m-%d-%H-%M-%S")
            ).total_seconds()
            > 0
        ):
            logger.info(f"Downloading new file - /{bucket_name}/{object_name}")
            received_bytes = io.BytesIO(
                client.get_object(bucket_name=bucket_name, object_name=object_name).data
            )

            # Delete existing cache to replace with a new one
            if existing_file_date != "":
                logger.info(
                    f"Deleting existing cache - {caching_service.get_cached_file_name(object_name)}"
                )
                Path.unlink(
                    caching_service.cache_path
                    / caching_service.get_cached_file_name(object_name)
