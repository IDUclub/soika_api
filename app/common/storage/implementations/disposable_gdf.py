...
class DisposableTextGDF(IDisposable):
    gdf: GeoDataFrame | None = None
    local_crs: CRS | None = None

    def try_init(self, bucket_name: str, object_name: str):
        logger.info(f"Trying to initialize TEXT GDF with /{bucket_name}/{object_name}")
        ...
        logger.info(f"TEXT GDF initialized with /{bucket_name}/{object_name}")
